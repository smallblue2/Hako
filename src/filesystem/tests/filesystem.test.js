...
describe("Filesystem tests", () => {
  ...
  beforeEach(async () => {
    ...
    await waitForCondition(async () => {
      ...
      await page.evaluate(async () => await window.Filesystem.initialiseFS());
    });
  });
  ...
  it("Check whether Filesystem object exists on the window object", async () => {
    const hasFilesystem = await page.evaluate(async () => {
      await window.Filesystem.initialiseFS();
  ...
  it("Check whether we can open a file (create)", async () => {
    const { error0, error1, error2, error3, fd, stat } = await page.evaluate(async () => {
      ...
      let { error: error3 } = window.Filesystem.remove("/persistent/test.txt");
  ...
  it("Confirm opening an existing file with create fails", async () => {
    const { error0, error1, fd0, fd1 } = await page.evaluate(async () => {
  ...
  it("Check whether closing a file works", async () => {
    const { error0, error1, error2, error3, error4, fd } = await page.evaluate(async () => {
      ...
      let { error: error4 } = window.Filesystem.remove("/persistent/another.txt");
    ...
    assert.ok(error4 == null, "Remove reported an error");
  });

  it("Check we can write to an open file", async () => {
    const { error0, error1, error2, error3,  fd } = await page.evaluate(async () => {
      ...
      let { error: error2 } = window.Filesystem.close(fd);
      let { error: error3 } = window.Filesystem.remove("/persistent/hello.txt");
  ...
  it("Check whether opening with only a read flag disallows writes", async () => {
    const { error0, error1, error2, error3, fd } = await page.evaluate(async () => {
      ...
      let { error: error2 } = window.Filesystem.close(fd);
      let { error: error3 } = window.Filesystem.remove("/persistent/another.txt");
      return { error0, error1, error2, error3, fd };
    });
  ...
  it("Confirm opening a non-existent file without create flag fails", async () => {
    const { error, fd } = await page.evaluate(async () => {
  ...
  it("Create a file, write to it, close it, re-open it, and read it using Read", async () => {
    ...
    const { error0, error1, error2, error3, error4, error5, error6, fd0, fd1, data, size } = await page.evaluate(async (content, len) => {
      ...
      let { error: error6 } = window.Filesystem.remove("/persistent/helloworld.txt");
      return { error0, error1, error2, error3, error4, error5, error6, fd0, fd1, data, size };
    }, content, len);
    ...
    assert.ok(error6 == null, "Remove reported an error");
  });
  ...
  it("Create a file, write to it, close it, re-open it, and read it using ReadAll", async () => {
    ...
    const { error0, error1, error2, error3, error4, error5, error6, fd0, fd1, data, size } = await page.evaluate(async (content) => {
      ...
      let { error: error5 } = window.Filesystem.close(fd1);
      let { error: error6 } = window.Filesystem.remove("/persistent/helloagain.txt");
      return { error0, error1, error2, error3, error4, error5, error6, fd0, fd1, data, size }
    }, content);
  ...
  it("Confirm opening protected system files for writing is blocked", async () => {
    const { error0, error1, error2, error3, error4, fd0, fd1 } = await page.evaluate(async () => {
      // Create system file
      ...
      window._FSM.FS.chmod("/persistent/systemfile", 0o700);
      let { error: error4 } = window.Filesystem.remove("/persistent/systemfile");
      return { error0, error1, error2, error3, error4, fd0, fd1 }
    });
  ...
  it("Get the stat of a file", async () => {
    const { error, stat } = await page.evaluate(async () => {
  ...
  it("Get the fdstat of a file", async () => {
    const { error0, error1, error2, fd } = await page.evaluate(async () => {
  ...
  it("Confirm a read-only file can't be written to", async () => {
    const { error0, error1, error2, error3, fd0, fd1 } = await page.evaluate(async () => {
  ...
  it("Confirm shifting an open file's cursor works", async () => {
    ...
    const { error0, error1, error2, error3, fd, data, size } = await page.evaluate(async (content, len) => {
  ...
  it("Confirm removing a file deletes it", async () => {
    const { error0, error1, error2, error3, error4, fd, stat0, stat1 } = await page.evaluate(async () => {
      let { error: error0, fd } = window.Filesystem.open("/persistent/cursor.txt", "crw");
      let { error: error1 } = window.Filesystem.close(fd);
  ...
  it("Move a file", async () => {
    const { error0, error1, error2, error3, error4, error5, fd } = await page.evaluate(async () => {
      ...
      let { error: error5 } = window.Filesystem.remove("/persistent/new.txt");
      return { error0, error1, error2, error3, error4, error5, fd }
    });
    ...
    assert.ok(error5 == null, "Remove reported an error");
  });
  ...
  it("Ensure user can't move a protected file", async () => {
    const { error0, error1, error2, error3, error4, error5, fd } = await page.evaluate(async () => {
      ...
      let { error: error3 } = window.Filesystem.stat("/persistent/old.txt");
      let { error: error4 } = window.Filesystem.stat("/persistent/new.txt");
      window._FSM.FS.chmod("/persistent/old.txt", 0o700);
      let { error: error5 } = window.Filesystem.remove("/persistent/old.txt");
      return { error0, error1, error2, error3, error4, error5, fd }
    });
  ...
  it("Ensure user can't overwrite a protected file with a move", async () => {
    const { error0, error1, error2, error3, error4, error5, fd0, fd1 } = await page.evaluate(async () => {
  ...
  it("Make a directory", async () => {
    const { error0, error1, stat } = await page.evaluate(async () => {
  ...
  it("Remove a directory", async () => {
    const { error0, error1, error2, error3, stat0, stat1 } = await page.evaluate(async () => {
      let { error: error0 } = window.Filesystem.make_dir("/persistent/mydir");
  ...
  it("Read a directory", async () => {
    const { error0, error1, entries } = await page.evaluate(async () => {
      let { error: error0 } = window.Filesystem.make_dir("/persistent/mydir");
      let { error: error1, entries } = window.Filesystem.read_dir("/persistent");
      console.log(JSON.stringify(window.Filesystem.read_dir("/persistent")));
    ...
    assert.deepStrictEqual(entries, [".", "..", "bin", "mydir"], "Read_dir returned incorrect entries");
  })

  it("Change active directory", async () => {
    const { error0, error1, error2, stat0, stat1 } = await page.evaluate(async () => {
